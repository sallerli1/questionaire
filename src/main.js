export default function Component(obj) {
    return new Promise((resolve, reject) => {
        ...
        component.beforeMount =
            obj.beforeMount instanceof Function
                ? obj.beforeMount
                : function() {};
        component.mounted =
...
function render(self, element, data, methods) {
    ...
    if (element.nodeType === Node.ELEMENT_NODE) {
        ...
        for (let i = 0; i < element.attributes.length; i++) {
            let matches;
            if (
                (matches = element.attributes[i].textContent
                    .trim()
                    .match(/({{\w+}})/g))
            ) {
                for (const match of matches) {
                    ...
                    element.setAttribute(
                        ...
                        element.attributes[i].textContent.replace(
                            match,
                            data[key]
                        )
                    );
            ...
            if (
                (matches = element.attributes[i].name
                    .trim()
                    .match(/^bind-(\w+)$/))
            ) {
    ...
    for (const one of fors) {
        if (data[one.dataset.for]) {
            for (let i = 0; i < data[one.dataset.for].length; i++) {
                ...
                if (
                    Object.prototype.toString.call(data[one.dataset.for][i]) ===
                    "[object Object]"
                ) {
